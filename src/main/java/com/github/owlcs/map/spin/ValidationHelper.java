...
public class ValidationHelper {
    ...
    private static class StringArg extends Arg {
        ...
        void testValue() {
            ...
            Exceptions.Builder error = error(Exceptions.FUNCTION_CALL_WRONG_ARGUMENT_STRING_VALUE);
            // anything:
            ...
            if (RDF.Property.equals(type)) {
                if (context != null && getFrameFunction().isMappingPropertyFunction()) {
                    if (!context.getSourceClassProperties().contains(value)) {
                        throw error(Exceptions.FUNCTION_CALL_WRONG_ARGUMENT_NON_CONTEXT_PROPERTY).build();
                    }
                }
            ...
            if (isDT(type) && value.canAs(OntProperty.class)) {
                ...
                if (ranges.stream().noneMatch(r -> match(type, r))) {
                    throw error(Exceptions.FUNCTION_CALL_WRONG_ARGUMENT_INCOMPATIBLE_RANGE).build();
                }
                return;
            }
            // unhandled situation:
            throw error(Exceptions.FUNCTION_CALL_WRONG_ARGUMENT_UNHANDLED_CASE).build();
        }
    }
    ...
    private static class CallArg extends Arg {
        ...
        void testChain() {
            ...
            testFunction(nested, model, context, Exceptions.FUNCTION_CALL_WRONG_ARGUMENT_FUNCTION_VALUE
        ...
        void testValue() {
            ...
            Exceptions.Builder error = error(Exceptions.FUNCTION_CALL_INCOMPATIBLE_NESTED_FUNCTION);
    ...
    private abstract static class Arg {
        ...
        Exceptions.Builder error(Exceptions code) {
            return code.create().addArg(argument).add(Exceptions.Key.ARG_VALUE, getValueKey());
        }
