...
import com.github.owlcs.map.utils.ClassPropertyMapListener;
...
public class MapModelImpl extends OntGraphModelImpl implements MapModel {
     ...
     * TODO: it is a generic method. Move to ONT-API ({@link com.github.owlcs.ontapi.jena.impl.UnionModel}?)
     *
    ...
    public MapModelImpl deleteContext(MapContext context) {
        ...
        if (!related.isEmpty()) {
            Exceptions.Builder error = error(Exceptions.MAPPING_CONTEXT_CANNOT_BE_DELETED_DUE_TO_DEPENDENCIES).addContext(context);
    ...
    public MapModelImpl bindContexts(MapContext left, MapContext right) {
        ...
        if (res.size() != 1) {
            Exceptions.Builder err = error(Exceptions.MAPPING_ATTACHED_CONTEXT_AMBIGUOUS_CLASS_LINK)
    ...
    public void validate(MapFunction.Call func) throws MapJenaException {
        ValidationHelper.testFunction(func, this, error(Exceptions.MAPPING_FUNCTION_VALIDATION_FAIL).addFunction(func).build());
    }

    protected Exceptions.Builder error(Exceptions code) {
        return code.create().add(Exceptions.Key.MAPPING, toString());
    }
